/*
** molecule_supplier.c -- a combined TCP+UDP server for:
...
void print_inventory(void); // prints atoms quantity
...
int main(int argc, char *argv[]) {
    ...
    // ---------------------------------------------------------
    // 4. Main select() loop to monitor TCP listen, UDP, and TCP clients
    // ---------------------------------------------------------
    while (1) {
    ...
    // We never reach here because of infinite loop
    close(tcp_listen_fd);  // for completeness
    close(udp_fd);
    return 0;
}
...
//are enough atoms to build that many molecules, and subtract
//the required atoms if possible (no molecule tracking)
