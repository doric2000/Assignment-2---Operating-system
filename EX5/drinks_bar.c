...
static void alarm_handler(int sig); //SIGALRM Handler
void print_inventory(void); // print the inventory of the drinks bar.

// Alarm signal handler: uses write (which is async-signal-safe) to print a message.
static void alarm_handler(int sig) {
    (void)sig;  // suppress unused parameter warning
    const char msg[] = ">>> Alarm handler invoked! <<<\n";
    // Use write instead of printf inside the handler
    write(STDOUT_FILENO, msg, sizeof(msg) - 1);
    // Optionally set a flag to let main() know the alarm fired
    timed_out = 1;
}
...
int main(int argc, char *argv[]) {
    ...
    //    Mandatory: -c <carbon> -o <oxygen> -h <hydrogen>
    //    Optional:  -t <timeout>
    //    Remaining args (after getopt): <tcp_port> <udp_port>
    // -----------------------------
    ...
    // Define long options and short option string
    static struct option long_opts[] = {
        {"carbon",   required_argument, 0, 'c'},   // -c <value> or --carbon=<value>
        {"oxygen",   required_argument, 0, 'o'},   // -o <value> or --oxygen=<value>
        {"hydrogen", required_argument, 0, 'h'},   // -h <value> or --hydrogen=<value>
        {"timeout",  required_argument, 0, 't'},   // -t <value> or --timeout=<value>
        {"tcp-port", required_argument, 0, 'T'},   // -T <port>  or --tcp-port=<port>
        {"udp-port", required_argument, 0, 'U'},   // -U <port>  or --udp-port=<port>
        {0, 0, 0, 0}
    };
    const char *short_opts = "c:o:h:t:T:U:";
    ...
    //check that both ports are acceptable and rly ports.
    if (tcp_port <= 0 || udp_port <= 0) {
        fprintf(stderr,
            "ERROR: both -T <tcp_port> and -U <udp_port> must be specified.\n"
            "Usage: %s -c <carbon> -o <oxygen> -h <hydrogen> "
            "[-t <timeout>] -T <tcp_port> -U <udp_port>\n",
            argv[0]);
        exit(EXIT_FAILURE);
    ...
    // 4) coverting ports to string for getaddrinfo:
    ...
    if ((rv = getaddrinfo(NULL, udp_port_str, &hints_udp, &servinfo_udp)) != 0) {
    ...
    // ---------------------------------------------------------
    while (1) {
        // if timeout flag is set (timeout seconds > 0 )
        if (timed_out) {
        ...
        if (ready < 0) {
            if (errno == EINTR) {
                continue;  // we will return to the begining of the while - might be sigalarm
            }
...
void parse_and_update_udp(const char *line, char *response, size_t resp_size) {
    ...
    // --------------------------------------------------------------
    // Print atoms and inventory after any “DELIVER …” request
    // --------------------------------------------------------------
    print_inventory();
